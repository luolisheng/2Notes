...
3. 默认缓存策略
	...
	3. 本地有缓存，检查缓存是否过期
		1. 没有过期则直接使用缓存
		2. 过期则发起请求，服务端对比资源Last-Modified/Etags。不同则返回新数据，否则返回304继续使用缓存。
	4. 使用默认缓存策略，客户端不用做任何设置，只需服务端响应头加上Cache-Control:max-age:xxx
4. 不想使用默认缓存策略，NSMutableURLRequest的cachePolicy:NSURLRequestReloadIgnoringCacheData，每次要请求服务器进行校验。
